...
class Question(models.Model):
    ...
    team_a = models.CharField(max_length=100)
    team_b = models.CharField(max_length=100)
    total_score = models.CharField('total score', null=True, blank=True, max_length=10)
    total_cards = models.CharField('total card', null=True, blank=True, max_length=10)
    penalty = models.BooleanField('Penalty', null=True, blank=True, default=False)
    status = models.CharField(max_length=10, choices=QUESTION_STATUS,
                              default='open')
    closed_at = models.DateTimeField(null=True, blank=True)
    created_at = models.DateTimeField(auto_now_add=True, db_index=True)
...
class Attempt(models.Model):
    user = models.ForeignKey(User, on_delete=models.CASCADE)
    question = models.ForeignKey(Question, on_delete=models.CASCADE)
    total_score = models.CharField(null=True, blank=True, max_length=10)
    penaltya = models.BooleanField('Penalty', null=True, blank=True, default=False)
    # new = models.BooleanField('New', null=True, blank=True, default=False)
    total_cards = models.CharField(null=True, blank=True, max_length=10)
    status = models.CharField(max_length=10, choices=ATTEMPT_STATUS, blank=True, null=True)
    created_at = models.DateTimeField(auto_now_add=True)
    ...
    def teams_score(self):
        if self.question.status == 'resulted':
            if self.total_score == self.question.total_score:
                return 'won'
            else:
                return 'lose'
        return ''
    ...
    def penalty(self):
        if self.question.status == 'resulted':
            if self.penaltya == self.question.penalty:
                return 'won'
            else:
                return 'lose'
        return ''
